...
const onFileChange = (event, path) => {
  const isJSON = path && path.length > 5 && path.substr(path.length - 5) === '.json'
  const isWireMockRecordedMapping = /\w*-\w*.json/ig.test(path)

  if (!isJSON || isWireMockRecordedMapping) {
    return
  }

  console.info('\x1b[33mFile change detected. Reseting Wiremock mappings ...\x1b[0m')
